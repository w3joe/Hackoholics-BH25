...
def main(onnx_path, engine_path, max_batchsize, opt_batchsize, min_batchsize, use_fp16=True, verbose=False)->None:
    ...
    # config.set_preview_feature(trt.PreviewFeature.FASTER_DYNAMIC_SHAPES_0805, True) # use this line for TensorRT 8.x, comment it out if using TensorRT 10.x
    ...
    config = builder.create_builder_config()
    # config.set_preview_feature(trt.PreviewFeature.FASTER_DYNAMIC_SHAPES_0805, True) # use this line for TensorRT 8.x, comment it out if using TensorRT 10.x
    # config.max_workspace_size = 10 << 30  # 1GB # use this line for TensorRT 8.x, otherwise
    config.set_memory_pool_limit(trt.MemoryPoolType.WORKSPACE, 10 << 30) # use this line for TensorRT 10.x
    ...
    # engine = builder.build_engine(network, config) # use this line for TensorRT 8.x, otherwise
    engine = builder.build_serialized_network(network, config) # use this line for TensorRT 10.x
    if engine is None:
    ...
    with open(engine_path, "wb") as f:
        # f.write(engine.serialize()) # use this line for TensorRT 8.x, otherwise
        f.write(engine) # use this line for TensorRT 10.x
